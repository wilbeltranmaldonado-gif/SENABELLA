document.addEventListener("DOMContentLoaded", function () {
  ...
  if (carrusel) {
    ...
    Object.assign(barra.style, { position: "absolute", bottom: "0", left: "0", height: "4px", width: "0%", backgroundColor: "#84b814", zIndex: "5", transition: "width 5s linear" });
    carrusel.style.position = "relative";
    carrusel.appendChild(barra);
    setTimeout(function () { barra.style.width = "100%"; }, 100);
    carrusel.addEventListener("slide.bs.carousel", function () {
      barra.style.transition = "none";
      barra.style.width = "0%";
      setTimeout(function () { barra.style.transition = "width 5s linear"; barra.style.width = "100%"; }, 50);
    });
  }
  ...
  function mostrarToast(mensaje, icono, tipo) {
    ...
    toast.innerHTML = '<i class="fa-solid ' + (icono || "fa-circle-check") + '"></i><span>' + mensaje + '</span><button class="toast-cerrar"><i class="fa-solid fa-xmark"></i></button>';
    ...
    setTimeout(function () { toast.classList.remove("toast-visible"); setTimeout(function () { toast.remove(); }, 300); }, 3500);
  }
  ...
  for (let i = 0; i < categorias.length; i++) {
    categorias[i].addEventListener("click", function () {
      ...
      for (let j = 0; j < categorias.length; j++) categorias[j].classList.remove("categoria-activa");
      if (!yaActiva) {
  ...
  function filtrarPorCategoria(cat) {
    ...
    for (let i = 0; i < cols.length; i++) {
      ...
      if (!cat || (cols[i].getAttribute("data-categoria") || "").indexOf(cat) !== -1) {
        card.style.opacity = "1"; card.style.transform = "scale(1)";
      } else {
        card.style.opacity = "0.3"; card.style.transform = "scale(0.95)";
      }
    }
  }

  // Botones Agregar al Carrito y Favorito en productos
  ...
  for (let i = 0; i < tarjetas.length; i++) {
    ...
    btnCarrito.addEventListener("click", function (e) {
      ...
      setTimeout(function () { btnCarrito.innerHTML = '<i class="fa-solid fa-cart-plus"></i> Agregar'; btnCarrito.classList.remove("btn-agregado"); }, 1500);
      ...
      if (contador) {
        ...
        setTimeout(function () { contador.classList.remove("contador-animado"); }, 400);
      }
    ...
    card.addEventListener("click", function () { abrirVistaRapida(card); });
  }
  ...
  modal.innerHTML =
    '<div class="modal-contenido"><button class="modal-cerrar"><i class="fa-solid fa-xmark"></i></button>' +
    '<div class="modal-cuerpo"><div class="modal-imagen"></div><div class="modal-info">' +
    '<h2 class="modal-nombre"></h2><p class="modal-precio"></p>' +
    '<div class="modal-rating"><i class="fa-solid fa-star"></i><i class="fa-solid fa-star"></i><i class="fa-solid fa-star"></i><i class="fa-solid fa-star"></i><i class="fa-solid fa-star-half-stroke"></i><span class="modal-rating-text">4.5 (128 reseñas)</span></div>' +
    '<p class="modal-descripcion">Producto de alta calidad disponible en Senabella. Envío gratis a todo Colombia.</p>' +
    '<div class="modal-cantidad"><label>Cantidad:</label><div class="modal-selector-cantidad"><button class="modal-btn-menos">−</button><span class="modal-num-cantidad">1</span><button class="modal-btn-mas">+</button></div></div>' +
    '<div class="modal-acciones"><button class="modal-btn-carrito"><i class="fa-solid fa-cart-plus"></i> Agregar al carrito</button><button class="modal-btn-comprar"><i class="fa-solid fa-bolt"></i> Comprar ahora</button></div>' +
    '<div class="modal-beneficios"><div><i class="fa-solid fa-truck-fast"></i> Envío gratis</div><div><i class="fa-solid fa-shield-halved"></i> Compra protegida</div><div><i class="fa-solid fa-rotate-left"></i> Devolución gratis</div></div>' +
    '</div></div></div>';
  ...
  modal.querySelector(".modal-btn-menos").addEventListener("click", function () { let n = parseInt(numCant.textContent) || 1; if (n > 1) numCant.textContent = n - 1; });
  modal.querySelector(".modal-btn-mas").addEventListener("click", function () { let n = parseInt(numCant.textContent) || 1; if (n < 20) numCant.textContent = n + 1; });
  ...
  modal.querySelector(".modal-btn-carrito").addEventListener("click", function () {
    ...
    let contador = document.querySelector(".contador-carrito");
    if (contador) { let num = (parseInt(contador.textContent.trim()) || 0) + cant; contador.textContent = " " + num + " "; }
  ...
  function cerrarModal() { modal.classList.remove("modal-visible"); document.body.style.overflow = ""; }
  document.addEventListener("keydown", function (e) { if (e.key === "Escape") cerrarModal(); });

  // Scroll Reveal
  let animados = document.querySelectorAll(".productos-grid .col, .promos-grid .col");
  for (let i = 0; i < animados.length; i++) { animados[i].classList.add("elemento-animado"); animados[i].style.transitionDelay = (i * 0.06) + "s"; }

  function verificarScroll() {
    let els = document.querySelectorAll(".seccion-animada, .elemento-animado");
    for (let i = 0; i < els.length; i++) { if (els[i].getBoundingClientRect().top < window.innerHeight * 0.88) els[i].classList.add("animado"); }
  }
  window.addEventListener("scroll", verificarScroll);
  verificarScroll();

  // Hover en tarjetas
  let todas = document.querySelectorAll(".categorias-inicio .col > div, .card");
  for (let i = 0; i < todas.length; i++) {
    todas[i].addEventListener("mouseenter", function () { todas[i].style.transform = "translateY(-6px)"; });
    todas[i].addEventListener("mouseleave", function () { todas[i].style.transform = ""; });
  }
  ...
  if (inputBusq && btnBusq) {
    function buscar() {
      ...
      let cols = document.querySelectorAll(".productos-grid .col");
      if (!t) { for (let i = 0; i < cols.length; i++) cols[i].style.display = ""; return; }
      let enc = 0;
      for (let i = 0; i < cols.length; i++) {
        if (cols[i].textContent.toLowerCase().indexOf(t) !== -1) { cols[i].style.display = ""; enc++; }
    ...
    inputBusq.addEventListener("keydown", function (e) { if (e.key === "Enter") buscar(); });
    inputBusq.addEventListener("input", function () {
      if (!inputBusq.value.trim()) { let c = document.querySelectorAll(".productos-grid .col"); for (let i = 0; i < c.length; i++) c[i].style.display = ""; }
    });
  }
  ...
  lightbox.innerHTML = '<div class="lightbox-contenido"><button class="lightbox-cerrar"><i class="fa-solid fa-xmark"></i></button><img src="" alt="" class="lightbox-img"></div>';
  document.body.appendChild(lightbox);
  lightbox.addEventListener("click", function (e) { if (e.target === lightbox || e.target.closest(".lightbox-cerrar")) lightbox.classList.remove("lightbox-visible"); });
  ...
  window.addEventListener("scroll", function () { btnUp.classList.toggle("btn-arriba-visible", window.scrollY > 400); });
